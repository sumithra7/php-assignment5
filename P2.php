...
if(isset($_POST['submit']))
{
	...
	if(isset($_POST['seat']))
		...
		echo '<script>alert("Please select your seat preference")</script>';
	if(isset($_POST['meal']))
		$meal=$_POST['meal'];
	else
	...
	$intoneday=60*60*24+time();
	setcookie("flight1",$name);
	setcookie("flight2",$seat);
	setcookie("flight3",$meal);
	setcookie("lastvisited",date("G:i - m/d/y"),$intoneday);
...
<?php
	...
	{	echo"<br>----------JSD Flights------------";
		...
		echo"<br>Last Visited:".$_COOKIE["lastvisited"];
